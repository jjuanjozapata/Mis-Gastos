        ...
        async function sincronizarColaOffline() {
            ...
            try {
                ...
                console.warn('Sincronización pausada por inestabilidad de red:', error.message);
            }
        }
...
let graficoInstancia = null;
        ...
        async function sincronizarCategoriasCache() {
            try {
                if (navigator.onLine) {
                    ...
                    if (!error && data) {
                        let idsEliminadas = JSON.parse(localStorage.getItem('categorias_eliminadas_ids') || '[]');
                        // La base de datos es la verdad absoluta para deletes reales, idsEliminadas cubre defaults
        ...
        async function aplicarAbonoDeuda(plan) {
            ...
            try {
                ...
                cerrarFlujo(); limpiar(); cargarPlanes(); cargarEstadisticas();
            } catch (err) {
  ...
  try {
    ...
    // Evitamos bloqueos si fallan las consultas secundarias
    ...
    // [NUEVO] Rescate Lie-Fi: Si el motor rechaza por Falsa Conectividad, forzar encolado local sin pérdida de datos
    ...
    // Fallo de lógica interna real (Autenticación, SQL, etc.)
    if (display) {
      ...
      mostrarToast('Almacenamiento casi lleno. Conéctate a internet para vaciar la cola.', 'error');
        ...
        async function cargarEstadisticas() {
            ...
            if (navigator.onLine) {
                ...
                // [CISO FIX] Removemos la restricción catData.length > 0 para permitir sobreescritura con arreglos vacíos
                if (!catError && catData) { 
                    const editadasLocal = categorias.filter(c => c._editado);
                    let categoriasFusionadas = catData.filter(c => !idsEliminadas.includes(c.id));
        ...
        function renderizarListaTransacciones(transacciones, mapaCat) {
            ...
            transacciones.forEach((t, index) => {
                ...
                if (index < 30) {
                    ...
                    /* [NUEVO] Inyección de clase rastreadora y dataset inmutable */
        ...
        function renderizarGraficoDonut(labels, data, centerText, customColors = null) {
            ...
            if (graficoInstancia) {
                // Mutación reactiva: actualizamos datos sin destruir el canvas (0 Memory Leaks, 100x más rápido)
